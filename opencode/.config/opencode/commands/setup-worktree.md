...
---
You are running the `/setup-worktree` command. This is a manual workflow for creating an isolated workspace.

If the user passed `$ARGUMENTS`, treat that as the feature or branch name if applicable.
...
2. derive a branch/worktree name from the feature if the user has not supplied one
