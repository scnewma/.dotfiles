...
---
You are running the `/brainstorm` command. This is a manual workflow. Use it only because the user explicitly invoked it.
...
Do not write code, scaffold files, invoke implementation commands, or take implementation actions until all of the following are true:
...
Create a todo item for each step and complete them in order:
...
The written spec should be precise enough that `/write-plan` can turn it into an implementation plan without having to rediscover the design.
...
Once the user approves the written spec, recommend `/write-plan` as the next command.
