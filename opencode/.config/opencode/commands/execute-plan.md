...
---
You are running the `/execute-plan` command. This is manual inline execution of a written plan.

If the user passed `$ARGUMENTS`, treat that as the plan path. Otherwise, identify the correct plan file before doing anything else.
...
Create a todo list with one entry per plan task.
...
After all tasks are complete and verified, run `/finish-development-branch`.

If the user would benefit from higher-quality task isolation and review gates, recommend `/execute-plan-with-subagents` instead of continuing inline.
